    ...
    public partial class PvInventariosPorGrupoPrint : Window
    {
        ...
        public void LoadInfo()
        {
            try
            {
                ...
                else
                {
                    ...
                    DataTable dt_tip = SiaWin.Func.SqlDT("select rtrim(cod_tip) as cod_tip,cod_tip+'-'+rtrim(nom_tip) as nom_tip from inmae_tip where estado=1 order by cod_tip ", "lineas", idEmp);
                    comboBoxTip.ItemsSource = dt_tip.DefaultView;
        ...
        private async void Ejecutar_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (comboBoxTip.SelectedIndex >= 0)
                {
                    ...
                    foreach (DataRowView ob in comboBoxTip.SelectedItems)
                    {
                        String valueTip = ob["cod_tip"].ToString();
                        _tipo += valueTip + ",";
                    ...
                    if (_tipo.Trim() != "")
                    {
                        ...
                        string ss = _tipo.Trim().Substring(_tipo.Trim().Length - 1);
                        if (ss == ",") _tipo = _tipo.Substring(0, _tipo.Trim().Length - 1);
               ...
               // }
                ...
                if (((DataSet)slowTask.Result).Tables[0].Rows.Count > 0)
                {
                    ...
                    //SiaWin.Browse(dsPrintSaldosGrupo.Tables[0], true);
        ...
        private bool CreaColumnas(int tipo)
        {
            ...
            if (tipo==1)
            {
                ...
                dataGridSF.Columns.Add(new Syncfusion.UI.Xaml.Grid.GridTextColumn() { Width = 280,HeaderText = "Nombre Producto", MappingName = "cod_ant" , AllowFiltering = true });
                ...
                dataGridSF.Columns.Add(new Syncfusion.UI.Xaml.Grid.GridCurrencyColumn() { Width = 90, HeaderText = "ValorUnitCli", MappingName = "val_uni", CurrencyDecimalSeparator = ".", CurrencyGroupSeparator = ",", CurrencyGroupSizes = System.Windows.Media.Int32Collection.Parse("3") });
            }
            return true;
        }
        ...
        private void dataGridSF_FilterChanged(object sender, Syncfusion.UI.Xaml.Grid.GridFilterEventArgs e)
        {
            var provider = (sender as Syncfusion.UI.Xaml.Grid.SfDataGrid).View.GetPropertyAccessProvider();
            var records = (sender as Syncfusion.UI.Xaml.Grid.SfDataGrid).View.Records;
            //Gets the value for frozen rows count of corresponding column and removes it from FilterElement collection.
            double valorCxC = 0;
            //TextTotalEntradas.Text = valorCxC.ToString(18, 2)
            for (int i = 0; i < (sender as Syncfusion.UI.Xaml.Grid.SfDataGrid).View.Records.Count; i++)
            {
                    valorCxC += Convert.ToDouble(provider.GetValue(records[i].Data, "saldo_fin").ToString());
            }
            TextTotalEntradas.Text = valorCxC.ToString("N");

        }

        private void Imprimir_Click_1(object sender, RoutedEventArgs e)
        {
            try
            {
                String Bod = TxtBodega.Text.ToString().Trim();
                if (Bod == "")
                {
                    MessageBox.Show("Falta Bodega ....");
                    return;
                }
                ...
                paramBod.Name = "Bod";
                paramBod.Values.Add(Bod);
                parameters.Add(paramBod);

                ReportParameter paramTip = new ReportParameter();
                paramTip.Values.Add(_tipo);
                paramTip.Name = "Tip";
                parameters.Add(paramTip);

                ReportParameter paramGru = new ReportParameter();
                paramGru.Values.Add(TxtGru.Text.Trim());
                paramGru.Name = "Gru";
                parameters.Add(paramGru);


                ReportParameter paramPrv = new ReportParameter();
                paramPrv.Values.Add("");
                paramPrv.Name = "Prv";
                parameters.Add(paramPrv);

                ReportParameter paramImpCodCLi = new ReportParameter();

                paramImpCodCLi.Values.Add("true");
                paramImpCodCLi.Name = "ImpCodCLi";
                parameters.Add(paramImpCodCLi);


                string TipoReporte = @"/Inventarios/TomaFisicaDeInvenarios(R766)";
                SiaWin.seguridad.Auditor(0, SiaWin._ProyectId, SiaWin._UserId, SiaWin._UserGroup, idemp, 0, 0, 0, "TomaFisicaDeInvenarios(R766) - Imprimio :" + Bod + " Fecha:" + paramfechaini + " - " +  " Reporte:" + TipoReporte, "");
                string TituloReport = "TomaFisicaDeInvenarios(R766) -";
        ...
        private void ImprimirValor_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                string TipoReporte = @"/Inventarios/TomaFisicaDeInvenarios(R766)Valores";
                SiaWin.seguridad.Auditor(0, SiaWin._ProyectId, SiaWin._UserId, SiaWin._UserGroup, idemp, 0, 0, 0, "TomaFisicaDeInvenarios(R766) - Imprimio :" + Bod + " Fecha:" + paramfechaini + " - " + " Reporte:" + TipoReporte, "");
                string TituloReport = "TomaFisicaDeInvenarios(R766) -";
                //public Reportes(List<ReportParameter> parameters, string reporteNombre, string TituloReporte = "", bool DirecPrinter = false, int Copias = 1, string PrintName = "", int ZoomPercent = 0, int idemp = -1)
                SiaWin.Reportes(parameters, TipoReporte, TituloReporte: TituloReport, Modal: true, idemp: idemp);
        ...
        private void ImprimirCND_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                String Bod = TxtBodega.Text.ToString().Trim();
                ...
                ReportParameter paramElaborado = new ReportParameter();

                paramElaborado.Values.Add("JESUS ANDRES NARVAEZ TRUJILLO");
                paramElaborado.Name = "Elaborado";
                parameters.Add(paramElaborado);

                string TipoReporte = @"/Inventarios/TomaFisicaDeInventarios(R987CND)";
                SiaWin.seguridad.Auditor(0, SiaWin._ProyectId, SiaWin._UserId, SiaWin._UserGroup, idemp, 0, 0, 0, "TomaFisicaDeInventarios(R987CND) - Imprimio :" + Bod + " Fecha:" + paramfechaini + " - " + " Reporte:" + TipoReporte, "");
                string TituloReport = "TomaFisicaDeInventarios(R987CND) -";
                //public Reportes(List<ReportParameter> parameters, string reporteNombre, string TituloReporte = "", bool DirecPrinter = false, int Copias = 1, string PrintName = "", int ZoomPercent = 0, int idemp = -1)
                SiaWin.Reportes(parameters, TipoReporte, TituloReporte: TituloReport, Modal: true, idemp: idemp);
                //-ReportCxC rp = new ReportCxC(parameters, TipoReporte);
                //parameters, @"/Contabilidad/Balances/BalanceGeneral"
        ...
        private void ImprimirPortal_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                String Bod = TxtBodega.Text.ToString().Trim();
                ...
                string TipoReporte = @"/Inventarios/TomaFisicaDeInventarios(R987PORTAL)";
                SiaWin.seguridad.Auditor(0, SiaWin._ProyectId, SiaWin._UserId, SiaWin._UserGroup, idemp, 0, 0, 0, "TomaFisicaDeInventarios(R987PORTAL) - Imprimio :" + Bod + " Fecha:" + paramfechaini + " - " + " Reporte:" + TipoReporte, "");
                string TituloReport = "TomaFisicaDeInventarios(R987PORTAL)) -";
                //public Reportes(List<ReportParameter> parameters, string reporteNombre, string TituloReporte = "", bool DirecPrinter = false, int Copias = 1, string PrintName = "", int ZoomPercent = 0, int idemp = -1)
                SiaWin.Reportes(parameters, TipoReporte, TituloReporte: TituloReport, Modal: true, idemp: idemp);
                //-ReportCxC rp = new ReportCxC(parameters, TipoReporte);
                //parameters, @"/Contabilidad/Balances/BalanceGeneral"
